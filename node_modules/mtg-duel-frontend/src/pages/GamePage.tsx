...
export default function GamePage() {
  ...
  const moveCardToPosition = useCallback((cardId: string, fromZone: string, toZone: string, x: number, y: number, isOpponent: boolean) => {
    if (!gameState) return
    setGameState(prev => {
      if (!prev) return prev
      const updated = { ...prev }
      const player = isOpponent ? updated.opponent : updated.player
      
      const fromArray = player[fromZone as keyof typeof player] as Card[]
      const toArray = player[toZone as keyof typeof player] as Card[]
      
      if (!Array.isArray(fromArray) || !Array.isArray(toArray)) return prev
  ...
  return (
    <div style={{
      ...
      <div className="h-12 bg-black/70 border-b border-gray-700 flex items-center justify-between px-6 relative">
      ...
      <div className="flex-1 flex flex-col overflow-hidden">
        <OpponentArea gameState={gameState} setContextMenu={setContextMenu} setShowLibraryMenu={setShowLibraryMenu} showLibraryMenu={showLibraryMenu} moveCard={moveCard} updateCardPosition={updateCardPosition} />

        <PlayerArea gameState={gameState} setContextMenu={setContextMenu} playCard={playCard} setShowLibraryMenu={setShowLibraryMenu} showLibraryMenu={showLibraryMenu} drawCard={drawCard} shuffleDeck={shuffleDeck} moveCard={moveCard} updateCardPosition={updateCardPosition} moveCardToPosition={moveCardToPosition} />
      </div>
...
function DraggableCard({ card, zone, setContextMenu, containerRef, updateCardPosition, isOpponent, index = 0 }: any) {
  const getInitialPos = () => {
    if (card.x !== undefined && card.y !== undefined) {
      return { x: card.x, y: card.y }
    }
    const x = (index * 80) % 400
  ...
  const [displayPos, setDisplayPos] = useState(getInitialPos())
  const cardRef = useRef<HTMLDivElement>(null)

  useEffect(() => {
    if (card.x !== undefined && card.y !== undefined) {
      setDisplayPos({ x: card.x, y: card.y })
    }
  }, [card.x, card.y])

  const handleMouseDown = (e: React.MouseEvent) => {
    if (e.button !== 0 || !cardRef.current || !containerRef.current) return
    e.preventDefault()
  ...
  useEffect(() => {
    ...
    const handleMouseUp = (e: MouseEvent) => {
      ...
      finalX = Math.max(0, Math.min(finalX, rect.width - cardWidth))
      finalY = Math.max(0, Math.min(finalY, rect.height - cardHeight))
      
      updateCardPosition(card.id, finalX, finalY, isOpponent)
      setIsDragging(false)
  ...
  }, [isDragging, dragOffset, containerRef, updateCardPosition, card.id, isOpponent])
  ...
  return (
    <div
      ...
      className="absolute w-14 h-20 bg-gray-700 rounded border border-gray-500 flex items-center justify-center overflow-hidden"
      style={{
        left: `${isDragging ? displayPos.x : card.x || 0}px`,
        top: `${isDragging ? displayPos.y : card.y || 0}px`,
...
function HandCard({ card, setContextMenu, playCard, gameState, containerRef, moveCardToPosition }: any) {
  const [isDragging, setIsDragging] = useState(false)
  const [dragOffset, setDragOffset] = useState({ x: 0, y: 0 })
  ...
  useEffect(() => {
    ...
    const handleMouseUp = (e: MouseEvent) => {
      ...
      moveCardToPosition(card.id, 'hand', 'battlefield', finalX, finalY, false)
      setIsDragging(false)
  ...
  }, [isDragging, dragOffset, containerRef, moveCardToPosition, card.id])
  ...
  return (
    <>
      <div
        ref={cardRef}
        onMouseDown={handleMouseDown}
        onClick={() => gameState.isPlayerTurn && isClickRef.current && playCard(card.id)}
        onContextMenu={(e) => {
          e.preventDefault()
          setContextMenu({ cardId: card.id, x: e.clientX, y: e.clientY, zone: 'hand' })
        }}
        className={`flex-shrink-0 w-14 h-20 rounded border-2 flex items-center justify-center text-xs overflow-hidden cursor-grab transition ${
      ...
      {isDragging && (
        <div
          ...
          className="rounded flex items-center justify-center text-xs overflow-hidden"
        >
          {card.imageUrl ? (
            <img src={card.imageUrl} alt={card.name} className="w-full h-full object-cover" />
          ) : (
            <div className="text-center text-purple-300 text-xs font-bold">{card.name}</div>
          )}
        </div>
      )}
    </>
...
function OpponentArea({ gameState, setContextMenu, setShowLibraryMenu, showLibraryMenu, moveCard, updateCardPosition }: any) {
    ...
    <div className="flex-1 flex gap-2 bg-black/20 p-2 overflow-hidden">
        ...
        <div className="flex-1 flex flex-col gap-1 min-h-0">
          ...
          <div className="flex-1 overflow-y-auto flex flex-col gap-1">
            {gameState.opponent.graveyard.slice(-3).map((card: Card) => (
              <SmallCardDisplay key={card.id} card={card} zone="graveyard" setContextMenu={setContextMenu} />
            ))}
          </div>
        </div>
        <div className="flex-1 flex flex-col gap-1 min-h-0">
          ...
          <div className="flex-1 overflow-y-auto flex flex-col gap-1">
            {gameState.opponent.exile.slice(-3).map((card: Card) => (
              <SmallCardDisplay key={card.id} card={card} zone="exile" setContextMenu={setContextMenu} />
            ))}
          </div>
        </div>
      </div>
      ...
      <div className="flex-1 flex flex-col gap-2 overflow-hidden">
        ...
        >
          ...
          {gameState.opponent.battlefield.map((card: Card, idx: number) => (
            <DraggableCard 
              key={card.id} 
              card={card} 
              zone="battlefield" 
...
function PlayerArea({ gameState, setContextMenu, playCard, setShowLibraryMenu, showLibraryMenu, drawCard, shuffleDeck, moveCard, updateCardPosition, moveCardToPosition }: any) {
  const containerRef = useRef<HTMLDivElement>(null)
    ...
    <div className="flex-1 flex gap-2 bg-black/20 p-2 overflow-hidden">
      ...
      <div className="flex-1 flex flex-col gap-2 overflow-hidden">
        ...
        >
          ...
          {gameState.player.battlefield.map((card: Card, idx: number) => (
            <DraggableCard 
              key={card.id} 
              card={card} 
              zone="battlefield" 
          ...
          <div className="flex gap-1 overflow-x-auto h-full">
            {gameState.player.hand.map((card: Card) => (
              <HandCard
                ...
                containerRef={containerRef}
                updateCardPosition={updateCardPosition}
                moveCardToPosition={moveCardToPosition}
              />
            ))}
          </div>
        </div>
...
function SmallCardDisplay({ card, zone, setContextMenu }: any) {
  return (
    <div
      onContextMenu={(e) => {
        e.preventDefault()
      ...
      className="w-full h-12 bg-gray-700 rounded border border-gray-500 flex items-center justify-center overflow-hidden text-xs hover:opacity-80"
    >
      {card.imageUrl ? (
